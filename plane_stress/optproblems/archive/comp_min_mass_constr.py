...
analysis = PlaneStressAnalysis(conn, dof, X, force, r0, qval, C,
    density=density, freqconstr=False)
...
if info != '':
    info = '-' + info
outputname = 'comp_min_mass_constr-{:s}-mass-{:.3f}-{:s}{:s}'.format(
    args.optimizer+extra, args.mass, prob_name, info)
if args.outdir != '':
