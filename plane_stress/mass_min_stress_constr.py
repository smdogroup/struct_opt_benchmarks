...
ys = analysis.ks_stress(xfull)

# We want to reduce maximum stress
ys = ys*0.9
analysis = PlaneStressAnalysis(conn, vars, X, force, r0, qval, C, ys=ys)
...
prob.model.add_constraint('topo.ks_stress', upper=1.0)

# Setup output format
outputname = '{:s}-{:s}-stress-iter-{:d}'.format(
    prob_name, args.optimizer, args.max_iter)
